...
//get
router.get('/',(req,res)=>{
    mysqlconexion.query('SELECT Id_salida, Fecha_salida, Motivo_salida, Imagen, Comentarios, Id_ganado FROM salidas',
    (error,rows,fields)=>{
        if(!error){
            res.json(rows);
        }
...
router.get('/:id', (req,res)=>{
    const {id} = req.params;
    mysqlconexion.query('SELECT Id_salida, Fecha_salida, Motivo_salida, Imagen, Comentarios, Id_ganado FROM salidas WHERE Id_salida=?', 
...
router.delete('/:id', (req,res)=>{
    const {id} = req.params;
    mysqlconexion.query('DELETE FROM salidas WHERE Id_salida=?',[id], (error,rows,fields)=>{
        if(!error){
            //res.json(rows);
            res.send({
                code:200,
